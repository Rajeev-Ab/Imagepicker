...
#define ASSET_WIDTH_HEIGHT ([self.view frame].size.width / 2) - 5
...
- (void)viewDidLoad
{
  ...
  CGRect rect = self.view.frame ;
   NSLog(@"width1 %f",rect.size.width);
...
{
  CGFloat width_height = ([self.view frame].size.width/2) - 5 ;
...
-(void)selectAssestAtIndexPath:(NSIndexPath* )indexPath
{
  ...
  RBImageCollectionCell* selectedCell =(RBImageCollectionCell*) [self.collectionView  cellForItemAtIndexPath:indexPath];
  selectedCell.contentView.backgroundColor = [UIColor greenColor];
    [self.selected_images setObject:asset forKey:indexPath];
...
- (void) collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(![self.selected_images_index containsObject:indexPath]){
      if (self.selected_images_index.count == 10){
        return;
      }
  ...
  RBImageCollectionCell* selectedCell =(RBImageCollectionCell*) [collectionView  cellForItemAtIndexPath:indexPath];
  selectedCell.transform = CGAffineTransformMakeScale(0.95,0.95);
